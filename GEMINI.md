...
This project, "Flux Modeler," is a desktop application for macOS and Windows that provides a graphical user interface (GUI) for the EnergyPlus building energy simulation program. It is built using Electron, which wraps a web-based front-end created with vanilla JavaScript, HTML, and CSS (utilizing TailwindCSS for styling). The application leverages Three.js for interactive 3D visualization of architectural scenes.

The core purpose of the application is to simplify and automate the entire daylighting and electric lighting analysis workflow. It allows users to parametrically model 3D scenes, import complex geometry, configure materials, design lighting systems, and run various EnergyPlus simulations. The application also features an integrated AI assistant to provide guidance and automate tasks.
...
*   **Renderer Process (Frontend):**
    ...
    *   **`scripts/simulation.js`**: Implements a "recipe-based" engine. It dynamically creates UI panels for different simulation types (e.g., illuminance, glare) and generates the corresponding EnergyPlus shell scripts.
    ...
    *   **`scripts/energyplus.js`**: Contains functions to generate EnergyPlus-specific file content (`.idf`, `.epw`, etc.).
    ...
    *   **`scripts/scriptGenerator.js`**: Generates the shell scripts (`.sh`, `.bat`) for running EnergyPlus simulations.
...
*   **Simulation Workflow:**
    ...
    5.  These scripts, along with EnergyPlus input files (`.idf`, `.epw`), are saved to a standardized folder structure on the user's local disk (e.g., `01_geometry`, `07_scripts`).
